...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Application.current_stage'
        db.add_column(u'applications_application', 'current_stage',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['companysettings.InterviewStage'], null=True),
                      keep_default=False)

        for application in orm.Application.objects.all():
            transitions = application.stage_transitions.all()
            if transitions:
                last_transistion = transitions[0]
                application.current_stage = last_transistion.stage
                application.save()


    def backwards(self, orm):
        # Deleting field 'Application.current_stage'
        db.delete_column(u'applications_application', 'current_stage_id')
